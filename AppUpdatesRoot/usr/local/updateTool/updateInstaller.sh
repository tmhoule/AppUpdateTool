...
notify(){
    message=$1
    if [ -f $bitBarApp/Contents/MacOS/BitBar ] && [ -f $bitBarPath/updateToolStatus.5m.sh ]; then
	BBisRunning=$(ps axuw |grep BitBar |grep -v grep)
	if [ -z "$BBisRunning" ]; then
	    open $bitBarApp #launch if not running
	fi
	...
	open -g bitbar://refreshPlugin?name=updateToolStatus.5m.sh
	if [ $? -ne 0 ]; then
	    killall BitBar
